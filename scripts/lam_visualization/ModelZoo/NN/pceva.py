...
class ApproximateVariancePooling(nn.Module):
    ''' 快速的求近似的方差pooling，不知道是不是真的近似，，，感觉应该比较近似 '''
...
class USF_PCEVA(nn.Module):
    ...
    def __init__(self, channels, sub_channels, zip_channels, blocks, scale, pre_conv=True):
        ...
        self.body = nn.Sequential(
            ECB(3 * 4, channels, 2),
            nn.PReLU(channels),
            *[PBlock(channels, sub_channels, zip_channels, pre_conv=pre_conv, iiii=_) for _ in range(blocks)],
...
class ESA(nn.Module):
    def __init__(self, n_feats, zip_channels, conv, iii):
    ...
    def forward(self, x):
        ...
        m = self.sigmoid(c4)
        import pickle
        pickle.dump(x*m,open(f'{self.iii}.pkl','wb'))

        return x * m


class PBlock_ESA(nn.Module):
    ''' PBlock '''
    def __init__(self, channels, sub_channels, zip_channels,iii, pre_conv=True):
        super().__init__()
        self.body = nn.Sequential()
        if pre_conv:
            self.body.append(nn.Conv2d(channels, channels, 1))
        ...
        for sub_channel in sub_channels:
            if sub_channel < channels:
                ...
                self.body.append(nn.PReLU(sub_channel))
        # 基于局部var信息的attention
        self.esa = ESA(channels, zip_channels, nn.Conv2d,iii)
...
class USF_PCEVA_ESA(nn.Module):
    ...
    def __init__(self, channels, sub_channels, zip_channels, blocks, scale, pre_conv=True):
        ...
        self.scale = scale
        self.body = nn.Sequential(
            nn.Conv2d(3 * 4, channels, 3, padding=1),
            nn.PReLU(channels),
            *[PBlock_ESA(channels, sub_channels, zip_channels, pre_conv=pre_conv,iii = _) for _ in range(blocks)],
            nn.Conv2d(channels, 3 * (scale * 2) ** 2, 3, padding=1),
        )
...
class PBlock_NO(nn.Module):
    ''' PBlock '''
    def __init__(self, channels, sub_channels, zip_channels, aaa, pre_conv=True):
        super().__init__()
        self.body = nn.Sequential()
        if pre_conv:
            self.body.append(nn.Conv2d(channels, channels, 1))
    ...
    def forward(self, input):
        ...
        out = self.body(input)
        import pickle
        pickle.dump(out,open(f'{self.aaa}.pkl','wb'))
        return out


class USF_PCEVA_NO(nn.Module):
    ''' PCEVA '''
    def __init__(self, channels, sub_channels, zip_channels, blocks, scale, pre_conv=True):
        super().__init__()
        ...
        self.body = nn.Sequential(
            nn.Conv2d(3 * 4, channels, 3, padding=1),
            nn.PReLU(channels),
            *[PBlock_NO(channels, sub_channels, zip_channels, pre_conv=pre_conv,aaa=_) for _ in range(blocks)],
            nn.Conv2d(channels, 3 * (scale * 2) ** 2, 3, padding=1),
        )
